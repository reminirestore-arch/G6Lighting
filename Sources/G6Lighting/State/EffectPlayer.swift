...
final class EffectPlayer: ObservableObject {
    @Published private(set) var currentFrame: LightingFrame = .off
    ...
    func play(_ effect: LightingEffect, onFrame: @MainActor @escaping (LightingFrame) async -> Void) {
        task?.cancel()
    ...
    func stop(showing frame: LightingFrame = .off) {
        task?.cancel()
        currentFrame = frame
    }
}
